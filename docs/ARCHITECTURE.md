...
- `MemoryBenchFixtureSeeder` 负责把真实或默认 bench cases 灌入 L0/L1/stable memory；`MemoryBenchRunner` 持久化 latest 和历史摘要，并输出 case drilldown；`GET /v2/gray/status` 把 mode、worker、candidate queue、recall audit 和 latest bench 汇总为灰度决策面板。
...
| POST | `/v2/carriers/projection/preview` | 预览 Carrier projection diff | v2 Phase 3 |
| POST | `/v2/carriers/projection/apply-preview` | 应用已审阅 projection preview | v2 Phase 3 |
...
| GET | `/v2/bench/history` | 读取 Bench 历史摘要 | v2 Phase 5 |
...
| POST | `/v2/ops/sensitive-candidates/reject` | 批量 reject 敏感候选，默认 retract promoted memory | v2 Ops |
| GET | `/v2/ops/sensitive-candidates/audit` | 查询敏感治理审计日志 | v2 Ops |
